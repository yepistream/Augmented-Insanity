...
/*!
 * Augmented Insanity additions: filesystem paths handed in from the
 * Android service so the native module loader can scan for .augins
 * packages and extract them into a writable cache.
 *
 * Both pointers are owned by android_globals and remain valid for the
 * lifetime of the process. Callers must NOT free.
 */
void
