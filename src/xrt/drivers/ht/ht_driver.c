...
static bool
ht_model_file_exists(const char *path)
{
                        ...
                        uint32_t view_count)
{
	...
	htd->async = t_hand_tracking_async_default_create(xfctx, sync, view_count);
	return htd;
}
                 ...
                 struct xrt_device **out_device)
{
	...
	if (create_info.models_dir != NULL && create_info.models_dir[0] != '\0') {
		// Caller supplied an override (e.g. an Aug-Ins module pointing at its
		// per-module extraction dir, where the .augins zip placed the models).
		int n = snprintf(path, ARRAY_SIZE(path), "%s", create_info.models_dir);
		if (n <= 0 || n >= (int)ARRAY_SIZE(path)) {
			U_LOG_E("models_dir override too long for buffer");
			return -1;
		}
	} else {
	...
	static const char *k_detection_model_name = "grayscale_detection_160x160.onnx";
	static const char *k_keypoint_model_name = "grayscale_keypoint_jan18.onnx";

	char detection_model_path[1024] = {0};
	char keypoint_model_path[1024] = {0};

	int detection_path_ret =
	    snprintf(detection_model_path, ARRAY_SIZE(detection_model_path), "%s/%s", path, k_detection_model_name);
	int keypoint_path_ret =
	    snprintf(keypoint_model_path, ARRAY_SIZE(keypoint_model_path), "%s/%s", path, k_keypoint_model_name);

	bool has_detection_model = detection_path_ret > 0 && detection_path_ret < (int)ARRAY_SIZE(detection_model_path) &&
	                           ht_model_file_exists(detection_model_path);
	bool has_keypoint_model = keypoint_path_ret > 0 && keypoint_path_ret < (int)ARRAY_SIZE(keypoint_model_path) &&
	                          ht_model_file_exists(keypoint_model_path);

	if (!has_detection_model || !has_keypoint_model) {
		char missing_models[256] = {0};
		size_t used = 0;

		if (!has_detection_model) {
			used += (size_t)snprintf(missing_models + used, ARRAY_SIZE(missing_models) - used, "%s%s",
			                         used > 0 ? ", " : "", k_detection_model_name);
		}
		if (!has_keypoint_model && used < ARRAY_SIZE(missing_models)) {
			used += (size_t)snprintf(missing_models + used, ARRAY_SIZE(missing_models) - used, "%s%s",
			                         used > 0 ? ", " : "", k_keypoint_model_name);
		}

		U_LOG_E("Hand-tracking models directory '%s' is missing required file(s): %s", path,
		        missing_models[0] != '\0' ? missing_models : "unknown");
		return -1;
	}

	sync = t_hand_tracking_sync_mercury_create(calib, create_info, path);

	const uint32_t view_count = create_info.view_count == 1 ? 1 : 2;
	struct ht_device *htd = ht_device_create_common(calib, false, xfctx, sync, view_count);
