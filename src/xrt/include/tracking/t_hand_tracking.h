...
struct t_hand_tracking_create_info
{
	...
	uint32_t view_count;                    //!< Number of active camera views, 0 means default stereo (2)
	float hand_size_m;                      //!< Target hand size in meters when hand_size_fixed is true
	bool hand_size_fixed;                   //!< Lock hand size at hand_size_m and disable runtime refinement
	bool optimize_hand_size;                //!< Enable/disable hand-size optimization when set
	bool optimize_hand_size_set;            //!< Whether optimize_hand_size should override default behavior
	//! Optional override for the directory containing the ONNX models. When NULL
	//! ht_device_create falls back to u_file_get_hand_tracking_models_dir().
	//! Used by Aug-Ins modules that bundle the models inside their .augins zip
	//! and pass their per-module extraction dir here.
...
struct t_hand_tracking_async *
t_hand_tracking_async_default_create(struct xrt_frame_context *xfctx,
