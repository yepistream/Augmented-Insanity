...
public class MonadoImpl extends IMonado.Stub {
    ...
    public MonadoImpl(@NonNull Context context) {
        ...
        // Hand the Augmented Insanity module loader the directories it needs
        // before starting the IPC server. modules/ holds .augins packages that
        // the user (or a future companion app) drops in; opennedmodules/ is
        // our writable scratch space for unpacked module trees.
        java.io.File modulesDir = new java.io.File(this.context.getFilesDir(), "modules");
        java.io.File cacheDir = new java.io.File(this.context.getCacheDir(), "opennedmodules");
        modulesDir.mkdirs();
        cacheDir.mkdirs();
        nativeSetAugInsPaths(modulesDir.getAbsolutePath(), cacheDir.getAbsolutePath());
    ...
    /**
     * Hand the Augmented Insanity module loader its filesystem paths.
     *
     * <p>Must be called before {@link #nativeStartServer} so the GRS can scan and
     * unpack any .augins packages before the IPC server accepts a client.
     *
     * <p>Implementation lives in {@code src/xrt/targets/service-lib/service_target.cpp}.
     *
     * @param modulesDir absolute path to the directory the GRS scans for *.augins files
     * @param cacheDir absolute path to a writable cache directory for unpacked modules
     */
    @SuppressWarnings("JavaJniMissingFunction")
    private native void nativeSetAugInsPaths(@NonNull String modulesDir, @NonNull String cacheDir);
