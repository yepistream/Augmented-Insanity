...
run_keypoint_estimation(void *ptr)
{
	...
	if (is_hand_explicit < hgt->tuneable_values.min_keypoint_presence_confidence.val) {
...
{
	if (wrap == nullptr || wrap->api == nullptr) {
		return;
	}

	if (wrap->meminfo != nullptr) {
		wrap->api->ReleaseMemoryInfo(wrap->meminfo);
		wrap->meminfo = nullptr;
	}
	if (wrap->session != nullptr) {
		wrap->api->ReleaseSession(wrap->session);
		wrap->session = nullptr;
	}
