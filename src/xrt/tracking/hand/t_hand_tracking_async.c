...
ht_async_mainloop(void *ptr)
{
	...
	// stay within their per-frame budgets. setpriority(0) here means "this
	...
	while (os_thread_helper_is_running_locked(&hta->mainloop)) {
		...
		const bool have_left = hta->frames[0] != NULL;
		const bool have_right = hta->cam_count <= 1 || hta->frames[1] != NULL;
